...
export const Auth: React.FC = () => {
  ...
  const DemoLandingPage = () => (
    <div className="w-full min-h-screen flex flex-col items-center justify-center p-4 md:p-8 text-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white animate-in fade-in duration-1000">
        <div className="absolute top-6 right-6 z-10"> {/* Ensure button is above other content */}
            <Button variant="outline" onClick={() => setShowLogin(true)} className="bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 border-white/30 shadow-lg text-xs">
                <LogIn className="mr-2 h-4 w-4" /> Login
            </Button>
        </div>

        <main className="max-w-5xl mx-auto mt-12 md:mt-0"> {/* Added margin-top for spacing from corner button on mobile */}
            ...
            <div className="mb-12 shadow-2xl rounded-xl overflow-hidden border-2 border-purple-500/50 transform hover:scale-105 transition-transform duration-300">
                <video 
                    ...
                    poster="YOUR_VIDEO_POSTER_URL.jpg" // Optional: Add a poster image
                />
            </div>

            <Button 
                onClick={goToExternalDemoSite} 
                size="lg" 
            ...
            <div className="text-left mb-12">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <FeatureCard
                        ...
                        description="Leverages Gemini API for intelligent email categorization into e-commerce specific labels like New Orders, Inquiries, and Returns."
                    />
                    <FeatureCard
                        icon={<BellRing size={28} className="text-blue-400" />}
                        title="Instant Discord Alerts"
                        description="Receive real-time Discord notifications for critical email types, ensuring prompt attention and action."
                    />
                    <FeatureCard
                        icon={<DatabaseZap size={28} className="text-green-400" />}
                        title="Reliable IMAP Sync"
                        description="Real-time email aggregation using IMAP IDLE, backed by cron-based polling for robust and dependable synchronization."
                    />
                    <FeatureCard
                        icon={<MessageSquareText size={28} className="text-pink-400" />}
                        title="RAG & Semantic Search"
                        description="Ongoing RAG implementation with a vectorized database and Elasticsearch for advanced semantic search capabilities (Coming Soon!)."
                    />
                     <FeatureCard
                        icon={<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-teal-400"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>}
                        title="Supabase Integration"
                        description="Utilizes Supabase Edge Functions for event processing and PostgreSQL for structured email data storage."
                    />
                    <FeatureCard
                        icon={<SearchCheck size={28} className="text-orange-400" />}
                        title="Modular Architecture"
                        description="Built with a Node/Express backend and TypeScript for a scalable and maintainable RESTful system."
                    />
                </div>
            </div>
